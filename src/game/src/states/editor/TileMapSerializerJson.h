...
private:
    ...
    void checkMapSize(const utils::Vector2i&,
                      const std::vector<std::vector<std::shared_ptr<TileInfo>>>&) const;
    ...
    std::vector<std::vector<std::shared_ptr<TileInfo>>> parseTiles(const nlohmann::json&,
                                                                   utils::Vector2i) const;
};
}