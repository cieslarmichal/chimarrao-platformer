...
public:
    std::unique_ptr<renderer_pool> create_renderer_pool()
    {
        return std::make_unique<sfml_renderer_pool>(
            std::unique_ptr<context_renderer>(unique_context_renderer.release()),
            std::unique_ptr<texture_warehouse>(unique_texture_warehouse.release()));
    }
    auto create_object_with_color(renderer_pool& shape_pool, const size2f& shape_size,
                                  const position2f& shape_position, const color& shape_color)
    {
        auto const shape_id = shape_pool.acquire(shape_size, shape_position, shape_color);
        return sfml_rectangle_shape{shape_id, shape_size, shape_position, shape_color};
    }

    auto create_object_with_texture(renderer_pool& shape_pool, const size2f& shape_size,
                                    const position2f& shape_position, const texture_path& path,
                                    const sf::Texture& texture)
    {
        auto const shape_id{shape_pool.acquire(shape_size, shape_position, path)};

        auto shape{
            sfml_rectangle_shape{shape_id, shape_size, shape_position, shape_pool.get_color(shape_id)}};
        shape.setTexture(&texture);
        return shape;
    }

    void expect_eq_position(const math::Position2f& expected, const math::Position2f& position)
    {
...
TEST_F(sfml_renderer_pool_test, acquireTwoRenderableObject_positionShouldMatch)
{
    ...
    auto const id1 = renderer_pool->acquire(dummy_size, position1, defaultColor);

    auto const position2 = math::Position2f{299.f, 1.f};
    auto const id2 = renderer_pool->acquire(another_dummy_size, position2, defaultColor);
...
TEST_F(sfml_renderer_pool_test, releasedObjectShouldNotBeDrawn)
{
    ...
    auto const second_shape{create_object_with_color(
        *renderer_pool, math::Size2f{13.f, 11.f}, math::Position2f{140.f, 177.f}, graphics::colors::green)};
...
TEST_F(sfml_renderer_pool_test, clearScreenBeforeDraw)
{
    auto renderer_pool = create_renderer_pool();
    auto const shape{
        create_object_with_color(*renderer_pool, dummy_size, dummy_position, graphics::colors::yellow)};
...
TEST_F(sfml_renderer_pool_test, createdObjectAfterReleaseOneShouldBeDrawn)
{
    ...
    auto const third_shape{create_object_with_color(*renderer_pool, math::Size2f{10.f, 12.f},
                                                    math::Position2f{100.f, 77.f}, graphics::colors::red)};
...
TEST_F(sfml_renderer_pool_test, acquiredObjectWithTextureShouldSetColorToWhite)
{
    ...
    EXPECT_CALL(*textures, get_access(default_texture_path)).WillOnce(ReturnRef(default_texture));
    auto const id = renderer_pool->acquire(dummy_size, dummy_position, default_texture_path);
