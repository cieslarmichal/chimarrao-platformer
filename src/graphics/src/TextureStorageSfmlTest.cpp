...
TEST_F(TextureStorageSfmlTest, getTextureShouldReturnTextureOnGoodPath) {
    ...
    EXPECT_CALL(*loaderPtr, load(_,goodPath))
        .WillOnce(DoAll(SetArgReferee<0>(texture), Return()));
...
{
    EXPECT_CALL(*loaderPtr, load(_,wrongPath))
        .WillOnce(Throw(TextureLoader::CannotAccess{"test"}));
