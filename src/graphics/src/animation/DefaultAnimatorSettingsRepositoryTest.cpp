...
public:
    ...
    std::unique_ptr<StrictMock<AnimatorSettingsReaderMock>> settingsReaderInit = std::make_unique<StrictMock<AnimatorSettingsReaderMock>>();
...
{
    EXPECT_CALL(*settingsReader, readAnimatorsSettings(settingsPath)).WillOnce(Return(emptyAnimatorsSettings));
